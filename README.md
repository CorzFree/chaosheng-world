...
拖动、触屏或方向键环顾，滚轮/双指缩放。“慢慢环顾”开启缓慢自动转动；“沉浸观看”隐藏界面，保留必要的来源信息。
...
- lib/travel-tools.ts：浏览器中的结构化旅行操作接口

## 验证

纯CPU测试覆盖影像文件、来源结构、六洲坐标、全景不镜像、地球拖动方向、筛选及存档校验。使用支持WebMCP的浏览器实际验证了工具注册、威尼斯全景环顾、伦敦切换、东京官方街景、返回威尼斯，以及非法视角和未知地点被拒绝。运行日志未报告错误。未进行整站截图视觉验收。
