...
- 每间小屋有一位居民，上午去海岸，中午找树荫，傍晚拜访邻居；夜里或雨中沿陆路回家。小船在真实码头之间往返，靠岸减速、收帆停留。
...
lib/world.ts 是纯模拟模型，lib/renderer3d.ts 是 Three.js 立体海图（地形、阴影、水深与太阳反光）；lib/renderer.ts 保留不支持 WebGL 2 的平面模式，lib/use-world.ts 连接交互与存档。lib/audio.ts 用 Web Audio 合成海浪和雨声。app/page.tsx 和 app/globals.css 提供响应式中文界面。tests/world.test.mjs 使用 Node 内置测试运行器。
...
默认使用立体地形。拖动平移，滚轮或双指缩放；右键拖动（或按住 Alt）转动镜头，地图右下的按钮也可改变方向与俯仰。实体脚下高度、水面的地形采样和鼠标落笔均使用同一三角地形。旧版 JSON 海图与当前设备存档继续可用。

新增纯 CPU 几何与行为回归检查：地形四边覆盖、坡面拾取一致、凹凸编辑高程、寻路禁止穿越水角、居民在真实树荫休息并回家避雨、小船真正抵达码头。未进行浏览器截图或 WebGL 运行时视觉验收。
