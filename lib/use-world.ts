...
export function useWorld() {
  ...
  function save() {
    ...
    try {
      localStorage.setItem(STORAGE_KEY, serialize(world.current));
      setSaved('已留在这台设备');
    } catch {
  ...
  function applyAt(x: number, y: number, force = false) {
    ...
    const oldId = w.id,
      ...
      m = paint(w, d.tool, x, y, view.current.radius);
    if (m && now - lastPaint.current > 200) notify(m);
    else if (m && force) notify(m);
    if (w.id !== oldId || w.revision !== oldRevision) {
      d.changed = true;
      d.paintX = x;
      d.paintY = y;
  ...
  useEffect(() => {
    ...
    const keys = (e: KeyboardEvent) => {
      ...
      if (/^[1-7]$/.test(e.key)) {
        choose(TOOL_IDS[Number(e.key) - 1]);
        e.preventDefault();
      }
